...
const sessionName = 'TURBO_panel_session';
const hostname = execSync('hostname', { silent: true }).toString().trim();

const systemIPs = [];
const terminalConfig = JSON.parse(readFileSync("/usr/local/turbo/web/storage/app/terminal/config.json").toString());
...
wss.on('connection', (ws, req) => {
    ...
    const file = readFileSync(`/usr/local/turbo/web/storage/app/terminal/sessions/${sessionID}`);
    if (!file) {
