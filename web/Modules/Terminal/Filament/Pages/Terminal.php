...
class Terminal extends Page
{
    ...
    protected function getViewData(): array
    {
        ...
        if (!is_dir('/usr/local/turbo/web/Modules/Terminal/nodejs/terminal/node_modules')) {
            $exec = shell_exec('cd /usr/local/turbo/web/Modules/Terminal/nodejs/terminal && npm install');
        }
        if (!is_dir('/usr/local/turbo/web/storage/logs/terminal')) {
            $exec = shell_exec('mkdir -p /usr/local/turbo/web/storage/logs/terminal/');
        }
        $exec = shell_exec('node /usr/local/turbo/web/Modules/Terminal/nodejs/terminal/server.js >> /usr/local/turbo/web/storage/logs/terminal/server-terminal.log &');
