...
class RunUploadBackupsToRemoteServers extends Command
{
     ...
     * @var string
     */
    protected $signature = 'turbo:run-upload-backups-to-remote-servers';
            ...
            foreach ($getRemoteBackupServers as $remoteBackupServer) {
                ...
                if ($findBackups->count() > 0) {
                    foreach ($findBackups as $backup) {
                        $uploadStatus = $remoteBackupServer->uploadFile($backup->filepath, 'turbo-system-backups');
                    }
                } else {
                ...
                if ($findHostingSubscriptionBackups->count() > 0) {
                    foreach ($findHostingSubscriptionBackups as $hostingSubscriptionBackup) {
                        $uploadStatus = $remoteBackupServer->uploadFile($hostingSubscriptionBackup->filepath, 'turbo-hosting-subscription-backups');
                    }
                } else {
