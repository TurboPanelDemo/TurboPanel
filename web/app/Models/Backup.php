...
class Backup extends Model
{
    ...
    {
        $cronJobCommand = 'turbo-php /usr/local/turbo/web/artisan turbo:run-backup-checks';
        ...
        $cronJobCommand = 'turbo-php /usr/local/turbo/web/artisan turbo:create-daily-full-backup';
        $findCronJob = CronJob::where('command', $cronJobCommand)->first();
        if (! $findCronJob) {
            $cronJob = new CronJob();
    ...
    public function checkBackup()
    {
        if ($this->status == BackupStatus::Processing) {
            ...
            if (file_exists($backupDoneFile)) {
                ...
                $validateTurboConfigFile = $tempValidatePath.'/turbo-config.ini';

                $errorsBag = [];
                if (! file_exists($validateDatabaseFile)) {
                    $errorsBag[] = 'Database file not found';
                }
                if (! file_exists($validateTurboConfigFile)) {
                    $errorsBag[] = 'Turbo config file not found';
                }
                if (count($errorsBag) > 0) {
                    $this->status = 'failed';
                    $this->backup_log = 'Backup failed. Database or turbo config file missing.';
                    $this->save();
                    return [
                        'status' => 'failed',
                        'message' => 'Backup failed. Database or turbo config file missing.',
                        'errors' => $errorsBag
                    ];
                }
                $originalTurboConfigContent = file_get_contents(base_path().'/turbo-config.ini');
                $backupTurboConfigContent = file_get_contents($validateTurboConfigFile);
                if ($originalTurboConfigContent != $backupTurboConfigContent) {
                    $errorsBag[] = 'Turbo config content mismatch';
                }

                if (count($errorsBag) > 0) {
                    $this->status = 'failed';
                    $this->backup_log = 'Backup failed. Database or turbo config file content mismatch.';
                    $this->save();
                    return [
                        'status' => 'failed',
...
//                $this->save();
    ...
    public function startBackup()
    {
        ...
        $backupFilename = 'turbo-backup-'.date('Ymd-His').'.zip';
        $backupFilePath = $storagePath.'/' . $backupFilename;

        // Export Turbo Panel database
        ...
        $shellFileContent = '';
        $shellFileContent .= 'echo "Backup Turbo Panel files"'. PHP_EOL;

        // Export Turbo Panel database
        $mysqlAuthConf = '/root/.turbo-mysql.cnf';
        $mysqlAuthContent = '[client]' . PHP_EOL;
        $mysqlAuthContent .= 'user="' . TurboConfig::get('MYSQL_ROOT_USERNAME') .'"'. PHP_EOL;
        $mysqlAuthContent .= 'password="' . TurboConfig::get('MYSQL_ROOT_PASSWORD') . '"' . PHP_EOL;
        file_put_contents($mysqlAuthConf, $mysqlAuthContent);

        $shellFileContent .= 'mysqldump --defaults-extra-file='.$mysqlAuthConf.' "'.TurboConfig::get('DB_DATABASE').'" > '.$databaseBackupPath . PHP_EOL;
        ...
        $backupStructure = [
            'database'=>$database,
            'config'=>TurboConfig::getAll()
        ];
        file_put_contents($backupTempPath.'/backup.json', json_encode($backupStructure, JSON_PRETTY_PRINT));
        $shellFileContent .= 'echo "Backup Turbo Panel Config"'. PHP_EOL;
        $shellFileContent .= 'cp '.base_path().'/turbo-config.ini '.$backupTempPath.'/turbo-config.ini'. PHP_EOL;
        ...
        if ($this->backup_type == 'full') {
            // Export Turbo Panel Hosting Subscription
        ...
        $shellFileContent .= 'turbo-php /usr/local/turbo/web/artisan turbo:run-backup-checks'. PHP_EOL;
