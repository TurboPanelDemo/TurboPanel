...
class CustomerResource extends Resource
{
    ...
    {
        $turboServers = [];
        $turboServers[0] = 'Main Server';
        $getTurboServers = TurboServer::all();
        foreach ($getTurboServers as $turboServer) {
            $turboServers[$turboServer->id] = $turboServer->name;
        }

        $schema = [];
        if ($getTurboServers->count() > 0) {
            $schema[] = Forms\Components\Select::make('turbo_server_id')
                ->label('Server')
                ->options($turboServers)
                ->default(0)
                ->hidden(function($record) {
                    if ($record) {
                        return $record->turbo_server_id == 0;
                    } else {
                        return false;
                    }
                })
