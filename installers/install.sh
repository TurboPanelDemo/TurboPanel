...
INSTALLER_URL="https://raw.githubusercontent.com/TurboPanelDemo/TurboPanel/main/installers/${DISTRO_NAME}-${DISTRO_VERSION}/install.sh"

INSTALLER_CONTENT=$(wget ${INSTALLER_URL} 2>&1)
if [[ "$INSTALLER_CONTENT" =~ 404\ Not\ Found ]]; then
    echo "TurboPanel not supporting this version of distribution"
    echo "Distro: ${DISTRO_NAME} Version: ${DISTRO_VERSION}"
    echo "Exiting..."
    exit 1
fi

# Check is Turbo is already installed
if [ -d "/usr/local/turbo" ]; then
    echo "TurboPanel is already installed. Exiting..."
    exit 0
fi

wget $INSTALLER_URL -O ./turbo-installer.sh
chmod +x ./turbo-installer.sh
bash ./turbo-installer.sh
